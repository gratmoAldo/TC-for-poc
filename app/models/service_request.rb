...
# TODO should look at all users who have the SR in their inbox
    ...
    :conditions => ["user_id=? and service_request_level>=?", owner_id, severity])
    if subscription
      device = APN::Device.find(:all, :conditions => ["token=?", subscription.token])
      if device.empty?
        app = APN::App.first ## TODO should look up be name
        if app
          device = APN::Device.create(:token => subscription.token,:app => app)
        end
      end
